#!/usr/bin/env python3
"""Strict deterministic aggregation for SGW-ESM Phase 3 experiments."""
...
COMPARISONS = (
    ("core_small_vs_redundant", "core_small", "sgw_redundant_final_only"),
    ("core_compute_matched_vs_redundant", "core_compute_matched", "sgw_redundant_final_only"),
    ("core_param_matched_vs_redundant", "core_param_matched", "sgw_redundant_final_only"),
    ("forced_final_vs_redundant", "sgw_redundant_final_only", "sgw_broadcast_forced_final_only"),
    ("forced_aux_vs_forced_final", "sgw_broadcast_forced_final_only", "sgw_broadcast_forced_aux_annealed"),
    ("core_compute_matched_vs_forced_aux", "core_compute_matched", "sgw_broadcast_forced_aux_annealed"),
    ("core_param_matched_vs_forced_aux", "core_param_matched", "sgw_broadcast_forced_aux_annealed"),
)
...
def load_causal(causal_dir: Path, expected_seeds: set[int]) -> list[dict[str, str]]:
    ...
    for path in paths:
        for row in read_csv(path):
            ...
            if condition != "sgw_broadcast_forced_aux_annealed":
...
def main() -> None:
    ...
    outputs: list[tuple[Path, list[dict[str, object]] | list[dict[str, str]]]] = [
        (output / "phase3_runs.csv", run_rows),
        (output / "phase3_summary.csv", summarize_runs(run_rows)),
        (output / "phase3_paired.csv", compare_runs(run_rows, args.bootstrap_replicates)),
        (output / "phase3_causal.csv", causal_rows),
        (
            output / "phase3_causal_comparisons.csv",
            compare_causal(causal_rows, args.bootstrap_replicates),
        ),
        (output / "phase3_routing.csv", routing_rows),
        (output / "phase3_routing_stability.csv", routing_stability_rows),
    ]
    ...
    manifest_path = output / "phase3_manifest.json"
