...
train_count=${6:-160}
holdout_count=${7:-160}
output_root=${8:-results/phase9-shard}
...
for ((offset=0; offset<seed_count; ++offset)); do
  ...
  for condition in "${conditions[@]}"; do
    args=(
      ...
      --batch-size "$batch_size" --train-count "$train_count"
      --holdout-count "$holdout_count"
      --output "$raw_dir/${condition}_seed${seed}.csv"
    )
    if [[ "$condition" == kv_delayed_annealed_curriculum ]]; then
      args+=(--causal-output "$causal_dir/causal_seed${seed}.csv")
    fi
    "./${build_dir}/sgw_train" "${args[@]}" >"$log_dir/${condition}_seed${seed}.log"
  done
done
